...
function apptivo_ecommerce_viewdetails_addtocart_btn($post_id,$price) {
	...
	$sizes = get_post_meta($post_id,'_apptivo_track_size',true);
	 // $sale_price = get_post_meta($post_id, '_apptivo_sale_price', true);
...
if (!function_exists('apptivo_ecommerce_template_loop_price')) {
	function apptivo_ecommerce_template_loop_price( $post, $_product ) {
		?>
<span class="price"><?php echo $_product->sale_regular_price_html(); ?>
</span>
		<?php
	}
}
...
if (!function_exists('apptivo_ecommerce_template_single_price')) {
	function apptivo_ecommerce_template_single_price( $post, $_product ) {
		?>
<p class="price">
<?php echo $_product->sale_regular_price_html(); ?>
</p>
<?php
	}
}
...
if (!function_exists('apptivo_ecommerce_simple_add_to_cart')) {
	function apptivo_ecommerce_simple_add_to_cart() {
		...
		if($itemId)
		{
			...
			// echo '<pre>';print_r($get_apptivo_item);
...
<div class="item_code">
	<label><?php echo apply_filters('apptivo_ecommerce_item_code_label','Item Code :');?>
	</label> <span class="app_item_code"><?php echo $get_apptivo_item->itemCode;?>
	</span>
</div>
		<?php endif; ?>

<!-- Price Details -->
		<?php  if(isset($get_apptivo_item->suggestedPrice)) :?>
		<?php
		...
		if($reagular_price_option == 'yes') {
			if($get_apptivo_item->suggestedPrice != '' && ($get_apptivo_item->suggestedPrice > 0 )) { ?>
<div class="regular_price">
	<label><?php echo apply_filters('apptivo_ecommerce_regular_price_label','Regular Price :');?>
	</label> <span><?php echo apptivo_ecommerce_price($get_apptivo_item->suggestedPrice);?>
	</span>
</div>
			<?php }
		}
...
<!-- Sale Price -->
		<?php  if($get_apptivo_item->itemPrice == '') { $get_apptivo_item->itemPrice = '0.00';  } ?>
<div class="sale_price">
	<label><?php echo apply_filters('apptivo_ecommerce_sale_price_label','Price :');?>
	</label> <span><?php echo apptivo_ecommerce_price($get_apptivo_item->itemPrice);?>
	</span>
</div>

		<?php
		if($get_apptivo_item->itemPrice <= 0)
		{
...
<form
	action="<?php echo esc_url( $_product->add_to_shopping_cart_url() ); ?>"
	class="cart" method="post">
	<?php if($tracksizes != '' && $tracksizes[0] != '') :?>
	<div class="tracking_colors">
		<span class="track_colors"><?php echo apply_filters('apptivo_ecommerce_tracking_color_label','Color :');?>
		</span> <select name="colors" id="colors">
		<?php foreach ($tracksizes as $sizes) :
		if( trim($sizes) != '') : ?>
			<option value="<?php echo $sizes; ?>">
			<?php echo $sizes; ?>
			</option>
			<?php endif;
			endforeach; ?>
	...
	<div class="tracking_sizes">
		<span class="track_sizes"><?php echo apply_filters('apptivo_ecommerce_tracking_size_label','Size :');?>
		</span> <select name="sizes" id="sizes">
		<?php foreach ($trackcolors as $colors) :
		if( trim($colors) != '') : ?>
			<option value="<?php echo $colors; ?>">
			<?php echo $colors; ?>
			</option>
			<?php endif;
			endforeach; ?>
	...
	<div class="item_quantity">
		<label><?php echo apply_filters('apptivo_ecommerce_qty_label','Quantity :');?>
		</label> <input name="quantity" value="1" size="2" title="Qty"
			class="input-text itemqty text" maxlength="2" />
	</div>
	...
	<button type="submit" class="btn alt">
		<span><?php _e($add_to_cart_label, 'apptivo-ecommerce'); ?> </span>
	</button>
...
<?php echo apply_filters('apptivo_ecommerce_cart_totals_label','<h2>Cart Total</h2>'); ?>
	<table cellspacing="0" cellpadding="0">
		<tbody>
			<!-- Sub total Amount.apptivo templatefun.php -->
			<tr>
				<th><?php _e('Subtotal', 'apptivo-ecommerce'); 
				 ?></th>

				<td><?php echo apptivo_ecommerce_price($cartInfo->subTotalAmount); //echo $apptivo_ecommerce->cart->get_cart_subtotal(); ?>
				</td>
			</tr>
			<!-- Slaes Tax Amount -->
			<?php if ($taxAmount > 0) : ?>
			...
			<tr>
				...
				<td><?php echo apptivo_ecommerce_price($cartInfo->shippingAmount); ?>
				</td>
			</tr>
			<?php endif;?>
			...
			<tr id="cart_shipping_options">
				<th><?php _e('Shipping', 'apptivo-ecommerce'); ?>
				</th>
				<td><?php
			...
			<?php endif; ?>
			<!-- Discount Amount -->
			<?php if ($cartInfo->totalDiscountAmount != '' && $cartInfo->totalDiscountAmount > 0) : ?>
			<tr class="discount">
				<th><?php _e('Discount', 'apptivo-ecommerce'); ?></th>
				<td>-<?php echo apptivo_ecommerce_price($cartInfo->totalDiscountAmount); ?>
				</td>
			</tr>
			<?php endif; ?>
			...
			<tr>
				...
				<td><strong><?php echo apptivo_ecommerce_price($cartInfo->totalPrice);  ?>
				</strong></td>
			</tr>
...
<form method="post" id="login" class="login"
		<?php if($page == 'checkout') { ?> style="display: none;" <?php } ?>>

	<?php if ($message) echo wpautop(wptexturize($message)); ?>
	<p class="login_heading">
	<?php echo apply_filters('apptivo_ecommerce_login_heading','Log In'); ?>
	</p>
	<p class="form-row form-row-first">
		<label for="username"><?php echo $account_username; ?> <span
			class="required">*</span> </label> <input
			value="<?php echo $_POST['username']; ?>" type="text"
			class="input-text" name="username" id="username" />
	</p>
	<p class="form-row form-row-last">
		<label for="password"><?php _e('Password', 'apptivo-ecommerce'); ?> <span
	...
	<p class="form-row">
		...
		<input type="submit" class="btn" name="login"
			value="<?php echo apply_filters('apptivo_ecommerce_login_submit_button','Login' ); ?>" />
	</p>
	<?php  if( $page == 'login' )
	{
...
<form method="post" class="reset_pwd" id="reset_pwd"
	style="display: none;">
	<p class="reset_pwd_heading">
	<?php echo apply_filters('apptivo_ecommerce_forgot_password_heading','Forgot Password');?>
	</p>
	<p class="form-row">
	<?php $apptivo_ecommerce->nonce_field('last_pwd', 'last_pwd') ?>
		<label for="username"><?php echo $account_username; ?> <span
			class="required">*</span> </label> <input type="text"
			class="input-text" name="account_username" id="account_username" /> <input
			type="submit" class="reset_btn" name="last_pwd"
			value="<?php echo apply_filters('apptivo_ecommerce_last_pwd_submit_button','Submit' ); ?>" />
	</p>
	<p>
		<a class="cancel_forgot_password" href="javascript:void(0)">Cancel</a>&nbsp;&nbsp;&nbsp;&nbsp;Not
		Yet Register? <a href="<?php echo $register_pageURL; ?>"> Register Now</a>
	</p>
</form>
	<?php
	}
...
if (!function_exists('apptivo_ecommerce_checkout_login_form')) {
	function apptivo_ecommerce_checkout_login_form() {
		...
		?>
<p class="info" id="checkout_login_reg">
<?php _e('Already registered?', 'apptivo-ecommerce'); ?>
	<a href="#" class="login_view"><?php _e('Click here to login', 'apptivo-ecommerce'); ?>
	</a>
</p>
<?php
...
if (!function_exists('apptivo_ecommerce_get_sidebar')) {
	function apptivo_ecommerce_get_sidebar() {
		...
		do_action('apptivo_ecommerce_before_sidebar');
		 // get_sidebar();
		$filepath = get_template_directory() . '/Sidebar.php' ;
