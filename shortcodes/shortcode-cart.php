...
function apptivo_ecommerce_cart() {
	...
	if(isset($_SESSION["last_seen_category"])){ $last_seen_category=$_SESSION["last_seen_category"]; }else{ $last_seen_category=get_permalink(get_option('apptivo_ecommerce_products_page_id')); }
	if (empty($shoppingCartLines[0])) :
		...
		do_action('apptivo_ecommerce_cart_is_empty');
                unset($_SESSION['zip_code']); //clear zip code
		echo '<p class="empty_cart_msg">'.apply_filters('apptivo_ecommerce_cart_empty_message','Your cart is Empty    ').'<a class="empty_cart_btn" href="'.$last_seen_category.'">'.apply_filters('apptivo_ecommerce_return_to_products_label','&larr; Continue Shopping').'</a></p>';
		return;
	endif;
	?>
<!--  Shopping Cart Form Details  -->	
<form action="<?php echo esc_url( $apptivo_ecommerce->cart->shopping_cart_url() ); ?>" method="post" id="awpcartpagefrm">
...
<?php echo '<div style="float:right;margin-bottom:10px;" class="">
           <a class="btn" href="'.$last_seen_category.'"><span>'.apply_filters('apptivo_ecommerce_continue_shopping_label','&larr; Continue shopping').'</span></a></div>
           <p style="margin:0px;padding:0px;clear:both;">            
           </p>'; ?>
    <div class="table-responsive tick-table">   
			...
			foreach ($shoppingCartLines as $cartLines) :
					...
					?>
						<tr>
							...
							<input rel="<?php echo $itm_qty; ?>" name="cart_item_qty[]" value="<?php echo esc_attr( $item_quantity ); ?>" iqty="<?php echo esc_attr( $item_quantity ); ?>" size="2" title="Qty" class="input-text itemqty text" maxlength="2" />
	...
	<div class="cart-collaterals col-sm-12 pad0">
	...
	<?php 
		$defaultFirmShippingMethodId = $shooping_cart->return->defaultFirmShippingMethodId;//Default Willcall Pickup ID.
                if(isset($_POST['zip_code'])){$_SESSION['zip_code']=$_POST['zip_code'];}
	?>
   <input type="hidden" id="wiicallpickup_enabled" name="wiicallpickup_enabled" value="<?php echo $defaultFirmShippingMethodId; ?>" />
	   <?php if(get_option ('apptivo_ecommerce_enable_remove_tax_and_shipping') != 'yes') {?>
			<tr id="shipping_zipcode" calls="tr_shipping_options">
				<td colspan="6" class="">
				 ...
				 <label for="willcallpickup"><?php echo apply_filters('apptivo_ecommerce_willcall_pickup_label' ,'Will call pickup'); ?></label> <br />
				 <input type="radio" <?php if( $_SESSION['apptivo_checkout_shipping_type'] != 'NO' ) { ?> checked="checked" <?php } ?> id="shipping" name="shippingoptions" value="shipping"  />					
						 ...
						 <input name="zip_code" class="input-text" id="zip_code" value="<?php if(isset($_SESSION['zip_code'])){echo $_SESSION['zip_code'];} ?>" /> 
