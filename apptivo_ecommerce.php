...
// error_log('message');
// error_log(print_r(get_option('plugin_error',true)));
...
function apptivo_ecommerce_frontend_scripts() {
	...
	$apptivo_ecommerce_params = array(
	    ...
	    //print recoeipt pag url for the thank page
	    'print_receipt_page_url'            =>get_permalink(get_option('apptivo_ecommerce_print_receipt_page_id')),
...
function my_plugin_deactivation() { 
	//delete pages
$pagesIds=array
('apptivo_ecommerce_thanks_page_id',
'apptivo_ecommerce_myaccount_page_id',
'apptivo_ecommerce_register_page_id',
'apptivo_ecommerce_logout_page_id',
'apptivo_ecommerce_login_page_id',
'apptivo_ecommerce_print_receipt_page_id',
'apptivo_ecommerce_products_page_id',
'apptivo_ecommerce_cart_page_id',
'apptivo_ecommerce_checkout_page_id',
'apptivo_ecommerce_secure_checkout_page_id'
);
    
       $myplugin_pages_ids=array();    
foreach( $pagesIds as $pagesId){ 
    $myplugin_pages = get_option($pagesId); 
array_push($myplugin_pages_ids,$myplugin_pages); 
if ( is_array($myplugin_pages_ids) && !empty($myplugin_pages_ids)) {
	foreach ($myplugin_pages_ids as $myplugin_pages_id) {
	  // wp_trash_post($myplugin_page);
		wp_delete_post($myplugin_pages_id);
		}
	}     
} 

global $wpdb; 
//if any mismatch value in the table for remove the page in the frondend
$wpdb->query( "DELETE FROM wp_posts WHERE post_content like '%[apptivo_ecommerce_%'" );
...
$wpdb->query( "DELETE FROM wp_posts WHERE post_content like '%[apptivo_ecommerce_%'" );
 ...
 $wpdb->query( "DELETE FROM wp_posts WHERE post_type ='item'");

$wpdb->query( "DELETE FROM wp_postmeta WHERE post_id NOT IN (SELECT id FROM wp_posts)" ); 
$wpdb->query( "DELETE FROM wp_postmeta WHERE meta_key LIKE '%_apptivo_ecommerce%'" );

$wpdb->query("DELETE FROM wp_options where option_name like '%apptivo_ecommerce_%'  "); 

}


add_action('wp_loaded','loggerfun');
