...
if (!isset($apptivo_ecommerce_loop['columns']) || !$apptivo_ecommerce_loop['columns']) 
...
$categoryID = get_post_meta($term->term_id, '_apptivo_category_id',$single = true);
	 if( $_GET['page'] == '' || $_GET['page'] < 1 )
	  ...
	  if(isset($_GET['s']) && $_GET['post_type'] == 'item') : //Search Item Ony
	  	...
	  	list($item_lists, $total_items_in_apptivo) = appv6_getItemsByCategoryId($mainCategoryID, $count_posts, $pageNo, $sortBy);
	...
	$apptivo_ecommerce_loop['loop'] = 0;
       ...
       if ($items_details->availableQuantityToTransact <= 0 && $items_details->enforceInvConstraint == 'Y') {
		...
		<li class="product <?php if ($apptivo_ecommerce_loop['loop']%$apptivo_ecommerce_loop['columns']==0) echo 'last'; if (($apptivo_ecommerce_loop['loop']-1)%$apptivo_ecommerce_loop['columns']==0) echo 'first'; ?>">
			...
			<?php
			//add to cart button.
			$item_lists = $product_postid."+".$items_details->itemId."+".$items_details->uomId."+1+".$items_details->itemPrice; // For posting Purpose... Format : itemID+itemUomID+itemqty+itemPrice
